...
export class SimulationService {
  ...
  async runSeasonMatchday() {
    try {
        ...
        if (!leagues || leagues.length === 0) {
            this.logger.warn('⚠️ No leagues found in DB! Cannot run simulation.');
            return { message: 'Database is empty. Please run /leagues/seed first.' };
        }
  ...
  // --- Start New Season (בחירת קבוצות חזקות) ---
  private async startNewSeason() {
    ...
    await this.teamModel.updateMany({}, {
      $set: { seasonStats: { matches: 0, points: 0, wins: 0, draws: 0, losses: 0, goalsFor: 0, goalsAgainst: 0 } }
    });
    ...
    if (championsLeague) {
        ...
        for (const league of nationalLeagues) {
            const top4 = await this.teamModel
                ...
                .sort({ attackStrength: -1 })
                .limit(4)
                .exec();
            
            if (top4.length > 0) {
                this.logger.log(`Qualifying from ${league.name}: ${top4.map(t => t.name).join(', ')}`);
    ...
    return { message: 'New Season Started! Top teams qualified based on strength.' };
  }

  // --- Simulate Single Match (עם הגנת קריסה) ---
